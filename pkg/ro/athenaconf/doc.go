...
// representation and overlays the parsed values onto an athenaconf.Config
// (a kernel-local identity-config target, not the application-layer config).
//
...
// configuration is always YAML loaded by the application-layer config package.
// The translator never appears in the hot path of any service.
package athenaconf
