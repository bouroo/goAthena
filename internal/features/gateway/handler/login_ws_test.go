...
// GetCharacterBySlot is the CH_SELECT_CHAR path. Login/char-list handler
// tests never drive char-select, so a missing fn returns success=false —
// any dispatch caller (handleCHSelectChar) then refuses cleanly instead
// of aborting the handshake on an Unimplemented status.
func (f *loginFakeIdentityClient) GetCharacterBySlot(ctx context.Context, req *identityv1.GetCharacterBySlotRequest, _ ...grpc.CallOption) (*identityv1.GetCharacterBySlotResponse, error) {
	_, _ = ctx, req
...
// VerifySession is the CZ_ENTER session-validation gate. Login/char-list
// handler tests never reach the map phase, so a default ok=true keeps any
// incidental dispatch path passing while staying a no-op.
func (f *loginFakeIdentityClient) VerifySession(ctx context.Context, req *identityv1.VerifySessionRequest, _ ...grpc.CallOption) (*identityv1.VerifySessionResponse, error) {
	_, _ = ctx, req
