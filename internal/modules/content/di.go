...
// clicks are a no-op until scripts are loaded. The WorldService is resolved
// lazily as the effect-builtins (warp/heal) world port; a resolve failure leaves
// world nil so the server still boots. The NPC store is exposed as the
